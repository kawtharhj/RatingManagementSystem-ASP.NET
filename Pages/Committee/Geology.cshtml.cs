...
{
    public class GeologyModel : PageModel
    {
        ...
        public GeologyModel(UserManager<UserData> usr, RoleManager<IdentityRole> identrole, RatingSystemDbContext db)
        {
            userManager = usr;
        ...
        {
            UserInfoComit = await dbContext.Users.Where(a => a.DoctorMajor == "Geology").ToListAsync();
        ...
        public async Task<IActionResult> OnPost(List<UserData> UserInfoComit)
        {
            ...
            foreach (var user in UserInfoComit)
            {
                ...
                if (user.isInCommittee == "true")
                {
                    ...
                    if (!userclaim.Any(c => c.Type == "Major" && c.Value == "Geology"))
                    {
                        ...
                        await userManager.AddClaimAsync(userID, new Claim("Major", "Geology"));
                    }
                    else if (userclaim.Any(c => c.Type == "Major" && c.Value == "Geology") && isInComt == null)
                    {
                ...
                else
                {
                    ...
                    if (existingMember != null)
                    {
                        ...
                        await dbContext.SaveChangesAsync();
                        var claimRemove = new Claim("Major", "Geology");
