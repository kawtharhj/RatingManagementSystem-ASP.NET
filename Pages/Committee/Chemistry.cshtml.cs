...
{
    public class ChemistryModel : PageModel
    {
        ...
        public ChemistryModel(UserManager<UserData> usr, RoleManager<IdentityRole> identrole, RatingSystemDbContext db)
        {
            userManager = usr;
        ...
        {
            UserInfoComit = await dbContext.Users.Where(a => a.DoctorMajor == "Chemistry").ToListAsync();
        ...
        public async Task<IActionResult> OnPost(List<UserData> UserInfoComit)
        {
            ...
            foreach (var user in UserInfoComit)
            {
                ...
                if (user.isInCommittee == "true")
                {
                    ...
                    if (!userclaim.Any(c => c.Type == "Major" && c.Value == "Chemistry"))
                    {
                        ...
                        await userManager.AddClaimAsync(userID, new Claim("Major", "Chemistry"));
                    }
                    else if (userclaim.Any(c => c.Type == "Major" && c.Value == "Chemistry") && isInComt == null)
                    {
                ...
                else
                {
                    ...
                    if (existingMember != null)
                    {
                        ...
                        await dbContext.SaveChangesAsync();
                        var claimRemove = new Claim("Major", "Chemistry");
