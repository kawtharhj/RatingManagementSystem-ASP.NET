...
namespace RatingManagementSystem.Pages.Committee
{
    ...
    public class MathModel : PageModel
    {
        ...
        public MathModel(UserManager<UserData> usr, RoleManager<IdentityRole> identrole, RatingSystemDbContext db)
        {
            userManager = usr;
        ...
        {
            UserInfoComit = await dbContext.Users.Where(a => a.DoctorMajor == "Math").ToListAsync();
        ...
        public async Task<IActionResult> OnPost(List<UserData> UserInfoComit)
        {
            ...
            foreach (var user in UserInfoComit)
            {
                ...
                if (user.isInCommittee == "true")
                {
                    ...
                    if (!userclaim.Any(c => c.Type == "Major" && c.Value == "Math"))
                    {
                        ...
                        await userManager.AddClaimAsync(userID, new Claim("Major", "Math"));
                    }
                    else if (userclaim.Any(c => c.Type == "Major" && c.Value == "Math") && isInComt == null)
                    {
                ...
                else
                {
                    ...
                    if (existingMember != null)
                    {
                        ...
                        await dbContext.SaveChangesAsync();
                        var claimRemove = new Claim("Major", "Math");
